...
siteNumber="403213"
discharge=australia(site=siteNumber,variable='discharge')
stage=australia(site=siteNumber,variable='stage')
...
kable(raw[1:5,],caption='Raw Australian gauge data')
```

## Site locations for all agencies can also be found:
```{r}
## brazilSites=brazil(sites=TRUE)
australiaSites=australia(sites=TRUE)
```
```{r,echo=TRUE}
## kable(brazilSites[1:10,],caption='Example Brazilian river gauge locations')
kable(australiaSites[1:10,],caption='Example Australian river gauge locations')
```
...
recent=australia(site=siteNumber,variable='stage',start_date = start,end_date = end)
