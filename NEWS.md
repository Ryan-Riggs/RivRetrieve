# RivRetrive 0.1.8

-   Updated Australia, France, Japan, and South Africa functions to interact with updated APIs.
-   Added functions to pull data from Ireland and Scotland.
-   Updated Canada script to pull from an API rather than using the tidyhydat package. This new function allows for real-time data access.

# RivRetrive 0.1.7

-   Updating USA gauge access using new dataRetrieval functionality and added Quebec gauge access.

# RivRetrive 0.1.6

-   Updating vignette to use US gauge data as the example.

# RivRetrive 0.1.5

-   This update changes the maintainer's email address to a permanent gmail account.

# RivRetrive 0.1.4

-   This update modifies the Australian retrieval code to fix a discrepancy between RivRetrieve and the Australian website.

# RivRetrive 0.1.3

-   This update adds informative error messages for gauges that have no available record and/or when the agency website it down.

# RivRetrieve 0.1.2

-   Added a `NEWS.md` file to track changes to the package.
